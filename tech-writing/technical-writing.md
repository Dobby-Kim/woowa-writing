...
• **ACID 모델**은 전통적인 데이터베이스 트랜잭션에서 사용하는 원칙입니다.
...
을 보장합니다.
은행 거래와 같은 강력한 일관성이 필요한 환경에서 주로 사용됩니다. 예를 들어, 은행에서 계좌 간 이체를 할 때, 돈이 중간에 사라지지 않도록 중간에 문제가 발생해도 모든 과정이 원자적으로 완료되거나 전혀 이루어지지 않도록 보장됩니다.
...
• 이렇듯 **BASE 모델**은 분산 시스템에서 자주 사용되는 방식입니다.
